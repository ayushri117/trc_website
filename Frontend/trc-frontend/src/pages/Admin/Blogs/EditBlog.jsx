    ...
    <div className="Add_Blog_Container">
      ...
      <Form className="blog_form" method="POST">
        <input
          ...
          name="title"
          placeholder="Title*"
          defaultValue={blogData ? blogData[0].title : ""}
        />
        <input
          type="text"
          name="date"
          placeholder="Date*"
          defaultValue={blogData ? blogData[0].date : ""}
        />
        <input
          type="text"
          name="auther"
          placeholder="Auther*"
          defaultValue={blogData ? blogData[0].auther : ""}
        />
        <input
          type="text"
          name="previewImage"
        ...
        <textarea
          ...
          cols={58}
          rows={10}
          defaultValue={blogData ? blogData[0].preview : ""}
        />
        <select name="resource" id="resource" className="blog_dropdown">
          {rescorceOption.map((resource) => {
            if (blogData[0].resourceRef === resource.value) {
              return (
                <option value={`${resource.value}`} selected="selected">
                  {resource.name}
                </option>
              );
            } else {
...
export async function action({ request, params }) {
  ...
  if (!response.data.ok) {
    return json(
      { error: true, message: "Could Not Edit Blog" },
      { status: 500 }
    );
  }
